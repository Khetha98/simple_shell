...
/**
 * _myenmy_environment - prints the current environment
...
/**
 * my_set_environment_variable - Initialize a new environment variable,
...
int my_set_environment_variable(info_t *info)
{
	if (info->argc != 3)
	{
		_print_error_string("Incorrect number of arguements\n");
		return (1);
	}
...
/**
 * my_unset_environment_variable - Remove an environment variable
 * @info: Structure containing potential arguments. Used to maintain
 *        constant function prototype.
 *  Return: Always 0
 */
...
int my_unset_environment_variable(info_t *info)
{
	int i;
